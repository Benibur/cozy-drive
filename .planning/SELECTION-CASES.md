...
**Vérifié par golden** (cas **H1/insert**, `corpus/H1/insert`, driver `capture-driver.js`) : le clone `[[w,x],[y,z]]` atterrit **juste après** le tableau du corps en haut (bloc #1), l'original intact — comme T3. Le tableau du **bas** (H-reg) et les cas partiels (H2/insert, H4/insert) atterrissent aussi correctement. **Résidu OUVERT distinct** : **H2/replace** corrompt la structure (2×2 → 1×2, ligne supprimée) — classe *replace cross tableau→¶*, `xfail`, à investiguer (peut être indépendant de la collision).
