...
- **chemin mixte cross tableau↔¶ au REPLACE** (`mixSelRange`, ~1583-1640 : `origStart/origEnd = mixSelRange.GetStartPos()/GetEndPos()`, 1619-1620, + `GetAllTables()` 1301) : **EXPOSÉ — CONFIRMÉ par H2/replace**. La plage brute à supprimer déraille sous la collision → une ligne du tableau du haut est détruite. L'audit « par index sûr » était **incomplet** : ce chemin est *positionnel*, pas indexé.
...
### 🔴 Résidu OUVERT confirmé — H2/replace (chemin replace cross tableau→¶)
**`H2/replace` corrompt** le tableau du haut (2×2 → 1×2, ligne 1 supprimée, texte injecté fusionné dans une cellule survivante). **Diagnostic isolé (2026-07-15), c'est la COLLISION, pas la géométrie** :
- même géométrie `C(1,1)→¶@end` replace sur `table-plain.docx` (sans en-tête) → **propre** (table 2×2 conservée, ligne remplacée in-place) ;
- sur le **même** `table-header.docx`, la table du **BAS** (immunisée, `[174,193]`) avec la même géométrie → **propre** ; seule la table du **HAUT** (`[0,30]`, collision) → corrompue.
C'est le **pendant côté replace du bug d'insertion** : l'insert a été corrigé (ancre basée-éléments) ; le chemin mixte replace utilise encore des **positions brutes** (§ audit ci-dessus, ~1583-1640). **Fix futur = basé sur les éléments, comme l'insert.** Golden désiré (in-place propre) à écrire ; sortie buggée archivée dans `corpus/H2/replace/capture.json`.
...
| H2 | `[T1.C(1,1),P1@end]` *(mixte : dernière ligne du tableau haut + ¶ suivant (OO aimante la ligne entière))* | ✅ copie réduite [Gamma\|DD] + ¶ Intro edit après le ¶ (comme T5) | ⚠️ in-place — ⚠️ CORRUPTION observée | ⚠️ · AXE H. OO aimante la sélection sur la ligne 1 ENTIÈRE (extraction [CELL:1,0]Gamma / [CELL:1,1]Delta + ¶). INSERT : copie réduite [Gamma\|DD] + Intro edit après le ¶, tableau haut intact (T5-like) → pass. REPLACE : ⚠️ CORRUPTION — tableau haut 2×2 → 1×2, ligne 1 (Gamma/Delta) SUPPRIMÉE, texte injecté fusionné dans une cellule → xfail. DIAGNOSTIC ISOLÉ 2026-07-15 = c'est la COLLISION §4quater (pas la géométrie) : même géométrie sur table-plain (sans en-tête) = propre ; sur le MÊME table-header, table du BAS immunisée = propre, seule la table du HAUT (collision) corrompt. Pendant côté replace du bug d'insertion : chemin mixte replace (~1583-1640) utilise des positions brutes ; fix futur = basé éléments. Golden désiré ÉCRIT (corpus/H2/replace/model.json, in-place propre, hand-authored) ; sortie buggée archivée corpus/H2/replace/capture.json. |
...
- [x] **§4quater — insertion après table en HAUT du corps** : ✅ RÉSOLU (`aa8772310`/build `2026-07-15.2`, ancre basée éléments). Fixture `table-header.docx` + goldens H livrés (`corpus/H1..H4,H-reg`). Vérifié : H1/insert clone juste après. RESTE : (a) **H2/replace** = corruption (`xfail`, golden désiré à écrire, à investiguer) ; (b) faire **bénir** par Ben les verdicts des goldens H (`verdict:pending`) + ajouter `after.docx`/captures d'écran ; (c) audit résiduel du site 2449 (itère aussi les tables d'en-tête — pas de crash observé, à confirmer).
