...
- **par index** (`tableDocIndices`, sites 1301 / 2209 / 2301 replace & partiel) : **sûrs** — l'index est cohérent entre extraction et injection (Replace validé).
- **par position** (site **1982**, cible de l'insertion « après le tableau » : `insSelEnd ∈ [tStart,tEnd]` puis `tEnd+1`) : **EXPOSÉ** — un `insSelEnd` bas matche d'abord une table d'en-tête → cible fausse.
...
### 🟡 Bug DÉFÉRÉ — insertion après un tableau en HAUT du corps
`Insérer` après sélection d'un tableau collé en haut atterrit **loin après** le tableau. Deux points de contact positionnels (l'ancre §5bis lignes ~830-869 **et** le site 1982) reposent sur des positions brutes qui restent **ambiguës** sous la collision. Un fix d'ancre (viser le début de l'élément suivant la table) calcule la bonne cible mais l'`InsertContent` réel atterrit quand même mal → **vrai fix = insertion basée sur les éléments, pas les positions** — changement profond dans la zone la plus fragile (harnais T/A) → **déféré**. À traiter avec un cas de test dédié (doc à en-tête-tableau).
...
| # | Sélection *(description)* | Insérer | Remplacer | État / limite |
|---|---|---|---|---|
| H1 | `[T1.full,T1.full]` *(tableau en HAUT du corps (doc à table d'en-tête/pied) — collision)* | ⚠️ clone après le tableau (comme T3) — §4quater: insertion basée positions ambiguë sous collision (déféré) | ✅ remplacement in-place du tableau (comme T3) | ⚠️ · AXE H (§4quater, collision de positions en-tête/pied). Fixture table-header — collision PROUVÉE par probeTables (en-tête [0,63] recouvre corps-haut [0,30] ; spuriousHeaderFooterHits=[0]). Ciblage VALIDÉ via probeTables : XXX atterrit dans la BONNE table (haut), pas de crash, pas de faux no_cell_match. INSERT = bug DÉFÉRÉ §4quater (insertion après tableau en haut du corps, positions ambiguës → fix = insertion basée éléments). Goldens complets (round-trip extraction) à capturer+bénir (Ben). |
| H2 | `[T1.C(1,1),P1@end]` *(mixte : cellule du tableau haut + ¶ suivant)* | ✅ partiel : chemin comme T5 (tableau→¶) | ✅ idem in-place | ✅ · AXE H. Probe insert : "Body intro paragraph XXX" (post-sélection localisée block:1) — ciblage correct sous collision. Golden complet à capturer+bénir. |
| H3 | `[T1.intra(0,0),T1.intra(0,0)]` *(une seule cellule du tableau haut → intra_cell)* | ✅ chemin paragraphe (insère dans la cellule) comme T1 | ✅ remplace le texte de la cellule comme T1 | ✅ · AXE H. Probe insert : "AlphaXXX" dans la cellule (0,0) — intra_cell correct (fix §4quater : intra_cell seulement si la sélection ENTIÈRE est dans la cellule). Golden à bénir. |
| H4 | `[T1.cells,T1.cells]` *(cellules partielles du tableau haut)* | ✅ copie réduite après le tableau (comme T2a) | ✅ in-place (comme T2a) | ✅ · AXE H. Probe insert (T1.C(0,0)..T1.C(1,1)) : bonne table ciblée, aucune misdirection vers l'en-tête. Golden complet (clone/round-trip) à capturer+bénir. |
| H-reg | `[T2.full,T2.full]` *(tableau en BAS (immunisé) — RÉGRESSION)* | ✅ clone après le tableau (comme T3) — table basse immunisée (positions hautes) | ✅ in-place (comme T3) | ✅ · AXE H régression : la table du corps EN BAS (positions [174,193] > fin en-tête [0,63]) NE collisionne PAS (probeTables : hors spuriousHeaderFooterHits). Probe insert : "XdXXX" (bonne cellule). Doit se comporter comme un T normal — garde que le fix §4quater ne casse pas les tables non-collision. Golden à bénir. |
...
- [ ] **§4quater — insertion après table en HAUT du corps (DÉFÉRÉ)** : `Insérer` atterrit loin après le tableau (collision de positions en-tête/pied ; ancre §5bis + site 1982 basés positions). Fix = insertion basée sur les **éléments**, pas les positions. Ajouter une **fixture à table d'en-tête** + goldens. Auditer le site 1982 et l'itération 2449 (traitent les tables d'en-tête).
