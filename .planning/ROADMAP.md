...
- [ ] **⑤(b) non-réduction du clone fusionné (T-reduc)** — RESTE OUVERT, **CADRÉ** : voir `.planning/T-REDUC-CADRAGE.md` (problème, contrainte Q4, 3 sémantiques S-A/S-B/S-C à trancher, options d'implémentation, risques spans, stratégie de test preuve-au-save). Recommandation : sémantique **S-A** (bounding-box aux frontières de fusion) + promotion en vraie phase gsd (`/gsd-review-backlog` → `/gsd-plan-phase`) ; alternative légitime **S-C** (statu quo assumé, coût nul, car ⑤a rend déjà la sélection correcte). **Décision produit en attente.**
