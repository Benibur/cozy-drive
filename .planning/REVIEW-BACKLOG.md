...
| **A6/insert post-sél off-by-one** | 🔴 **Bug figé dans un golden BÉNI**, rendu visible par `selMarkup` : la post-sélection d'A6/insert avale **1 caractère du suffixe hôte** — `«Second e»r flows` au lieu de `«Second »er flows`. Le chemin **replace est correct** (`«Second »er flows`) → il y a un modèle juste à côté. Confirmé par 2 mesures indépendantes (`GetRangeBySelect().GetText()` d'OO + parcours position par caractère). **Miroir intra-cellule `Ac6/insert` identique** (béni ce matin aussi). Le fix A6-postsel du build `.7` a réduit le bug (`block3:20`→`10`) sans l'éliminer. | corriger `mergedTrailingLen` sur le chemin **insert** (s'aligner sur le replace) | 🔴 **À CORRIGER** |
