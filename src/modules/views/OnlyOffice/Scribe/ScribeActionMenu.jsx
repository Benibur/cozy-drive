...
const ScribeActionMenu = forwardRef(
  ({ onSelect, onClose, onOpenPanel }, ref) => {
    ...
    const actions = useMemo(() => {
      ...
      const base = SCRIBE_ACTIONS.map(action =>
        action.id === 'translate'
          ? { ...action, children: translateChildren }
          : action
      )
    ...
    const activeParent =
      isMobile && activeSubmenu
        ? actions.find(a => a.id === activeSubmenu)
        : null
    ...
    const handleCustomLangSubmit = useCallback(() => {
      ...
      if (trimmed) {
        const translateLabel = t(
          actions.find(a => a.id === 'translate').labelKey
        )
      ...
      >
        <Paper
          ...
          style={{
            ...
            outline: 'none',
            borderRadius: isMobile ? 0 : MENU_CORNER,
          ...
          {isMobile && activeParent ? (
            ...
            actions.map((action, index) => (
              ...
              >
                <MenuRow
                  selected={
                    (focusIndex === index && !activeSubmenu) ||
                        ...
                        child.type === 'input' ? (
                          <MenuRow
                            ...
                            onMouseEnter={() => {
                              ...
                              if (customLangRef.current)
                                customLangRef.current.focus()
                            }}
                            onClick={() => {
                              if (customLangRef.current)
                                customLangRef.current.focus()
            ...
            <>
              <div
                ...
                style={{
                  ...
                  backgroundColor:
                    focusIndex === PROMPT_INDEX ? focusedBg : undefined,
              ...
              {hasOpenPanel && (
                <MenuRow
                  ...
                  onMouseEnter={() => {
                    if (!mouseMoveEnabledRef.current) return
                    setFocusIndex(OPEN_PANEL_INDEX)
                    focusMenu()
                  }}
                >
                  <MenuRowIcon>
