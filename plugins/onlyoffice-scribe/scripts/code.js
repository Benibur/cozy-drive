  ...
  function buildAndInject(md, mode, fallbackHtml) {
    ...
    window.Asc.plugin.callCommand(function() {
      ...
      function createTableFromMarkers(cells) {
        ...
        try {
          var mt = Api.CreateTable(maxC + 1, maxR + 1);
          if (!mt) return null;
          ...
          log("TABLE rebuilt from markers " + (maxR + 1) + "x" + (maxC + 1) + " (no snapshot, no original)");
          return mt;
        } catch (ctErr) {
          log("createTableFromMarkers failed: " + ctErr);
          return null;
        }
      }
