(function(window, undefined) {
  ...
  function onSelectionExtractResult(resultJson) {
      ...
      if (result && result.empty) {
        ...
        castSelectionGeometry(null, false);
      ...
      // Under-selection floating button: emit lightweight geometry ALWAYS (even when
      // the side panel is closed / not subscribed) so the button tracks the live
      // selection independently. Rect is null for image-only selections (asc_Get-
      // SelectionBounds is text-only) → button stays hidden there (MVP).
  ...
  // Api.GetSelectionScreenRect); the host adds the editor iframe offset.
