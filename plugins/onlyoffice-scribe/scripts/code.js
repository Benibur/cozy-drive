(function(window, undefined) {
  ...
  function buildAndInject(md, mode, fallbackHtml) {
    ...
    window.Asc.plugin.callCommand(function() {
      ...
      return JSON.stringify({ fullTableInsert: (Asc.scope.__fullTableInsert === "1") });
    }, false, scribeInjectRecalc, function(ret) {
      // Deferred whole-table re-selection for a full-table INSERT: the doc has now recalculated,
      // so doc.GetRange over the inserted table's corner cells covers ALL cells (inside the
      // injection callCommand it stopped one cell short — the freshly-inserted table's positions
      // weren't settled yet). Re-select corner-to-corner off the current selection's start.
      try {
        var _fti = ret ? JSON.parse(ret) : null;
        if (_fti && _fti.fullTableInsert) reselectFullTableAfterInsert();
      } catch (eFti) {}
      callbackFired = true;
