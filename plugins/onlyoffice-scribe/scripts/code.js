(function(window, undefined) {
  ...
  var SCRIBE_BUILD = "2026-07-23.1 — LOGS : cette chaine (le changelog complet) n'est plus deversee dans la console. log(\"build ...\") et le log d'init n'affichent plus que le TOKEN de version (SCRIBE_BUILD.split(\" \")[0]) ; surtout, le log d'init se declenchait a CHAQUE changement de selection (config initOnSelectionChanged) => il est desormais monte dans la garde `!toolbarButtonAdded` et ne s'affiche qu'UNE fois. Aucune autre logique changee. — 2026-07-22.3 — le trigger-intent hote->plugin transporte desormais un champ source, que le plugin RENVOIE tel quel dans les donnees de l'intent AI_TEXT_ASSISTANT (le plugin n'a aucun avis dessus). Motif : l'hote doit distinguer une pression CLAVIER (Ctrl+Maj+I, qui FERME le panneau lateral s'il est ouvert) d'un clic sur le BOUTON sous-selection (qui doit au contraire ouvrir son menu PAR-DESSUS le panneau ouvert, le bouton restant desormais visible en permanence). — 2026-07-22.2 — UN CLIC N'EST PAS UNE SELECTION : le bouton flottant sous-selection APPARAISSAIT BRIEVEMENT a chaque clic de deplacement du curseur. Cause : OO laisse Selection.Use arme sur un simple clic (Selection_SetStart au mousedown, Start===End au mouseup), donc le patch sdkjs — qui calcule la geometrie sous IsSelectionUse() — rapportait la boite du CURSEUR (largeur nulle), et hasText valait \"il y a un rect\" = vrai. Fix : castSelectionGeometry filtre desormais sur IsSelectionEmpty() du logic document (lu en meme-origine, sans callCommand), avec repli sur le test largeur>0 si les internes bougent. Une image selectionnee reste couverte : CDrawingsController.IsSelectionEmpty renvoie false. — 2026-07-22.1 — DOCUMENT_GEOMETRY : le plugin pousse desormais la geometrie de la ZONE DOCUMENT (et de la PAGE) a l'hote, pour poser le bouton flottant \"panneau\" dedans au lieu de le deviner. Motif : Drive ne peut mesurer aucun element de chrome OO (autre origine) et les pages sont dessinees sur un CANVAS (aucun noeud DOM par page) ; une constante en dur est fausse des que l'utilisateur bascule le ruban compact. Source : id_viewer / id_viewer_overlay sont 2 controles freres ancres aux 4 cotes du meme conteneur (HtmlPage.js) => leur box EST la zone document, ruban/regles/bande d'icones de droite exclus. Lisible en DOM simple car cette iframe de plugin est MEME ORIGINE que la page editeur et creee SANS attribut sandbox (common/plugins.js) => aucun patch sdkjs et surtout aucun callCommand (qui tronquerait le redo). La box PAGE (plus etroite que le viewer) vient en refinement de m_arrPages + ConvertCoordsToCursor3(...,true) — internes NON publics, donc try/catch + null, l'hote retombant sur la box viewer. Rafraichissement 100% evenementiel (ResizeObserver sur id_viewer = resize/ruban compact/panneau droit ; asc_onZoom/asc_onZoomChange pour le zoom qui ne redimensionne PAS le viewer mais la page) — pas de polling : le setTimeout d'une iframe de fond est bride. — 2026-07-21.6 — bouton sous-selection : la geometrie est desormais POUSSEE par le patch sdkjs (event onSelectionGeometryChanged) au lieu de rider l'extraction lourde debouncee. Motif : le chemin piggyback arrivait ~1s en retard (iframe background = setTimeout bride + extraction complete) et ne pouvait PAS suivre un scroll/zoom (aucun changement de selection). Tirer un callCommand leger plus souvent etait exclu : callCommand tronque le redo. Le push ne coute aucun callCommand. Le chemin piggyback reste en FALLBACK pour un SDK non patche, et se tait des le premier event recu (sinon un rect perime ecraserait un rect frais). — 2026-07-21.5 — under-selection floating button: emit lightweight SELECTION_GEOMETRY {rect,hasText} (rect from the new sdkjs patch Api.GetSelectionScreenRect, piggybacked on the extraction callCommand so redo stays intact), independent of the panel. Rebase du chantier bouton-sous-selection sur feat/scribe-in-right-panel (build .4). — 2026-07-21.4 — BUG PREEXISTANT TROUVE AU PASSAGE : Api.CreateTable prend (LIGNES, COLONNES) et non (colonnes, lignes). Le code passait Api.CreateTable(nCols, nRows) => tout tableau NON CARRE levait 'Row index N is out of bounds' et faisait echouer TOUTE l'injection (le texte autour compris, pas seulement le tableau). Invisible jusqu'ici car tous les cas testes etaient CARRES (2x2). Prouve en live : un markdown 3 colonnes x 2 lignes n'inserait RIEN ; args inverses => 2 lignes x 3 colonnes, contenu A/B/C + 1/2/3 correct. Corrige aux 2 sites (chemin markdown-pipe + createTableFromMarkers). — 2026-07-21.3 — PANNEAU LATERAL (fidelite) : un tableau reinjecte depuis le chat etait reconstruit A PLAT (fusions/largeurs/polices perdues) faute de snapshot. L'extraction de CONTEXTE (mode document) produit desormais aussi les snapshots ToJSON(true,true) de chaque tableau, indexes par le MEME compteur que les marqueurs [TABLE:N], et les renvoie a l'hote dans la reponse cozy-bridge:document-extracted. ToJSON est une operation de LECTURE : elle ne viole pas la contrainte 'le contexte ne mute pas le document' (celle qui interdit SetName), et elle preserve les FUSIONS (gridSpan/vMerge) qu'une reconstruction depuis les seuls marqueurs ne peut pas restituer. Cote hote (View.jsx), les snapshots sont conserves et RENVOYES au clic Inserer/Remplacer, FILTRES aux seuls [TABLE:N] presents dans le fragment — le canal retour PANEL_ACTION est plafonne a 1 Mo (validateIntent), contrairement au canal d'extraction qui le contourne. reconstructTable retrouve alors un snapshot et restitue le tableau A L'IDENTIQUE, meme si le document a change depuis. Le rebuild a plat (createTableFromMarkers) reste le DERNIER recours. — 2026-07-21.2 — PANNEAU LATERAL (images) : une image d'un fragment disparaissait SILENCIEUSEMENT a l'Inserer/Remplacer. Cause : l'extraction de CONTEXTE (mode 'document') n'appelle jamais SetName (elle ne doit pas muter le document), mais emettait quand meme des marqueurs '![IMG:scribe-img-N]' — des handles FICTIFS que AUCUN dessin ne porte (verifie live : contexte annoncant scribe-img-1/2/3 sur un document a 2 dessins TOUS SANS NOM). Le LLM les renvoie, injectDrawingInto ne resout rien, et le paragraphe image entier etait jete. Fix en 2 temps : (1) en mode document on n'emet plus de handle reinjectable mais un token descriptif '[image]' — le modele sait qu'il y a une image, on ne distribue plus de reference qu'on ne sait pas honorer (l'extraction de SELECTION, elle, fait SetName : ses marqueurs restent reels et inchanges) ; (2) filet de securite a l'injection : un marqueur image non resolu laisse desormais un paragraphe VISIBLE '[image]' (italique) + un log, au lieu d'une disparition muette. — 2026-07-21.1 — PANNEAU LATERAL : les boutons Inserer/Remplacer d'un fragment contenant un TABLEAU perdaient le tableau (le texte autour passait). Cause : le contexte du chat est extrait en mode 'document', qui emet des marqueurs [TABLE:N]/[CELL:r,c] SANS produire tableSnapshots ni tableDocIndices (seule l'extraction de SELECTION les materialise) => reconstructTable n'avait ni snapshot ni table d'origine et renvoyait null => les 3 sites d'appel faisaient `continue` et le placeholder etait saute SILENCIEUSEMENT. Fix : createTableFromMarkers() construit un tableau neuf a partir des seuls marqueurs (dimensions = max r/c + 1, style aligne sur le chemin markdown-pipe) ; reconstructTable prend les cellules en 3e argument et l'utilise en dernier recours ; la branche REPLACE gagne un cas !origTable (rien a modifier en place => on insere le tableau reconstruit au lieu de le jeter). La mise en forme d'origine (polices/largeurs/fusions) n'est pas recuperable sur ce chemin : fidelite du CONTENU preferee a la perte silencieuse. Les tableaux markdown en pipes empruntaient deja un chemin fonctionnel (Api.CreateTable) et sont inchanges. — 2026-07-20.1 — POST-SEL insert de clone PARTIEL FUSIONNE (T2b/T2c, bug (5) du blessing Ben) : la post-selection tombait sur le HAUT du clone (rows 0-2) au lieu des cellules EDITEES (T2b row 3 Hi/Hj ; T2c rows 1-2). Cause : reselectFullTableAfterInsert (fix (4)) selectionnait TOUT le clone coin-a-coin via doc.GetRange(positions), qui casse en travers des fusions (L#2) => s'arretait une cellule trop court. En fait la post-sel de l'insert PARTIEL vient de selectByRefs (selectionne tout le clone), PAS de reselectFullTableAfterInsert (jamais appele : content ne finit pas par une table). Fix band-aid (a) : la branche merged partial insert enregistre selectedCellCoords dans une var LOCALE, RETOURNEE par le callCommand (partialInsertCells) — les ecritures Asc.scope DANS un callCommand ne propagent pas a un callCommand frere ; le callback plugin republie ces cellules sur Asc.scope.__partialInsertCells PUIS appelle reselectFullTableAfterInsert, qui (flag present) selectionne le RECTANGLE min/max r,c des cellules dans le clone via ExpandTo (element-based, compose a travers les fusions) au lieu du tableau entier. Le clone reste COMPLET (§4bis reduction sous fusion = chantier T-reduc separe, non traite). Non-regression visee : T3/H1 (whole-table insert) + T2a (partial non-fusionne, reduit) + T2b/T2c replace inchanges. — 2026-07-19.4 — POST-SEL full-table insert omettait la DERNIERE cellule (BUG, remonte au blessing Ben) : l'insert d'un clone de tableau ENTIER selectionnait N-1 des N cellules (T3/T9/H1/H-reg insert : «w»«x»«y» sans z ; T9 : …«GammaI» sans DeltaI). Cause REELLE (sondee en direct) : le ref TABLE dans content[] est INFIABLE pour extraire un range apres InsertContent — selectLast.GetRange() ET l'acces aux cellules sur ce ref tombent une cellule trop court ; re-fetcher le MEME tableau FRESH via doc.GetAllTables() + ancrer sur les cellules d'angle donne bien tout le tableau (sonde : cell(0,0).1er¶ .ExpandTo( cell(dern,dern).dernier¶ ) = 'w\\tx\\r\\ny\\tz\\r\\n'). Fix : quand selectLast est un tableau, localiser le tableau insere FRESH par la position de depart fiable (startRange), puis ancrer start = 1er ¶ de la cellule haut-gauche, end = dernier ¶ de la cellule bas-droite. Debloque l'axe H (H1+H-reg avaient le meme defaut). Le build .11 (cellules sur le ref STALE) n'avait rien change. — 2026-07-17.10 — EXTRACTION cellule -1 caractere (BUG PROD) : toute selection dans une cellule HORS DERNIERE COLONNE envoyait au LLM son texte ampute du dernier caractere (Alpha -> Alph, ...brown fox -> ...brown fo). Cause : para.GetText() d'un ¶ de cellule finit par un TAB terminateur (Alpha\\t) que ni les runs ni range.GetText() ne contiennent ; l.4457 ne strippait que \\r\\n -> paraText 'Alpha\\t' (6) plus long que rangeText 'Alpha' (5) -> la selection ENTIERE de la cellule est prise pour PARTIELLE -> clipEnd=1 -> dernier char coupe. Derniere colonne finit par \\r\\n (deja strippe) -> epargnee (Beta OK). Fix : strip du \\t terminateur de paraText ET rangeText, GATE intra-cellule (GetParentTableCell) pour ne pas couper un tab tape en fin de ¶ normal (que les runs contiennent). Meme famille que le fix A3 (\\r\\n). Sonde LIVE .9 : T1.C(0,0)@start..@end -> text 'Alpha' md 'Alph' ; @start..@mid -> text 'Al' md 'A'. Invisible du corpus car extractedMd capture mais jamais oracle. N'affecte AUCUN model.json (l'injection utilise la fixture md, pas l'extraction) -> oracle 34/34 inchange, 62 goldens valides. — 2026-07-17.9 — A6/Ac6 insert post-sel off-by-one : la selection avalait 1 caractere du suffixe HOTE (<<Second e>>r flows au lieu de <<Second >>er flows) ; fige et BENI dans les goldens A6/insert + Ac6/insert, invisible sous la forme end:{block:3,offset:10} — revele par selMarkup (build .5). Cause : l'ancre de FIN etait ARITHMETIQUE (selectLast.GetEndPos() - mergedTrailingLen) en unites de position OO, qui comptent les frontieres de runs — donc la LITIERE DE RUNS VIDES laissee en quantite VARIABLE par les chemins d'injection (A6 : 1 run vide en insert, 2 en replace, pour le meme texte final) decale le resultat ; le replace tombait juste par hasard. Fix : ancre REELLE mergedTrailingStart (= fin du ¶ AVANT l'append, capturee a la fusion) — meme remede que la sentinelle du chemin inline (cf commentaire l.1936 : 'positions count run boundaries -> off-by-N'). — ORACLE selection : dumpState emet selText (le texte couvert par la selection) + selMarkup (ce texte en situ, encadre << >>). Les unites de position {block,offset} restent emises mais sont DEMONETISEES (debug only, plus dans le modele normalise). Motif : offset compte les frontieres d'elements RUNS VIDES INCLUS, que paraToBlock saute et que normalizeModel refiltre -> les 2 champs sont en desaccord sur ce qui est du bruit (la litiere de runs vides du chemin replace decale les nombres sans rien changer a l'ecran) ; et un nombre opaque est imbenissable (le golden A6/insert a fige, beni, une post-selection avalant 1 caractere de l'hote). — REGRESSION table-insert : cleanupTrailingBlockPara DETRUISAIT le ¶ suivant le tableau insere (T3/H1 'Outro paragraph' perdu). La branche de fusion A6 (build .4) se gardait sur blocks[last] = le ¶ PLACEHOLDER SCRIBE-TABLE-n (plain) au lieu de content[last] = le TABLEAU reel -> appendRunsPreserving no-op sur un tableau PUIS RemoveElement du ¶ hote. Fix : garde lastIsTable (meme patron qu'insSimpleInline l.2177 / isSimpleInline l.2233) -> la branche de fusion et la propagation de style sont sautees quand le dernier element injecte est un tableau. Regression LATENTE depuis le build 2026-07-16.4 (merge A6) : l'axe tableau n'avait pas ete rejoue depuis. — 2026-07-17.3 — T-intra axe A : hookSetSelection supporte aussi `.P<m>` + multi-¶ intra-cellule (pour les captures before.png). — 2026-07-17.2 — T-intra axe A : grammaire driver `T<n>.C(r,c).P<m>@kind` (¶ m-ieme d'une cellule) + resolution multi-¶ intra-cellule (ExpandTo de 2 offsets par-¶) dans le test-hook -> permet A5/A6 DANS une cellule. — 2026-07-17.1 — T-intra axe A complet : replace-mode smart-spacing rendu cell-aware (hostAt via findHostParaAt) — corrige 'The quickXXX'/'XXXquick' (espace de collage manquant au REPLACE intra-cellule, A2/A4 replace). Fixture table-arules.docx (cellule-phrase + cellule 3-¶). — 2026-07-16.12 — T-intra V2 : re-collapse para-relatif GATE intra-cellule seulement (top-level garde doc.GetRange(insPos,insPos) — sinon +2 sur la post-sel inline A2/A4). — 2026-07-16.11 — T-intra V2 : fix curseur @end dernier ¶ de cellule (GetText renvoie 'texte\\t' -> l'offset @end depassait le texte des runs -> fallback offset 0 = XXXAlpha). Strip du \\t terminateur de cellule dans host-detection + test-hook. — 2026-07-16.10 — T-intra V2 (regles A intra-cellule) : host-detection cell-aware (findHostParaAt descend dans les cellules) -> smart-spacing + bord->nouveau ¶ (insCaretAtEnd) s'appliquent intra-cellule ; re-collapse du curseur via hostPara.GetRange(off,off) (para-relatif, fiable en cellule) au lieu de doc.GetRange(insPos,insPos) absolu (retombait a 0 -> XXXAlpha). — 2026-07-16.9 — T-intra V3 (harnais dev-hook) : dumpState.locate situe desormais une selection INTRA-CELLULE -> {block:<idx table>, cell:{r,c}, cellBlock:<¶ dans la cellule>, offset} (avant : block:-1, post-sel intra-cellule aveugle). Inert en prod (flag-gated dumpState). — 2026-07-16.8 — T8/intra-cell corruption : l'injection multi-¶ (chemin bloc) DANS une cellule aspirait le ¶ top-level apres le tableau (Outro) dans la cellule. Cause : cleanupTrailingBlockPara/cleanupLeadingSpacer scannaient doc.GetElement (top-level) -> traversaient la frontiere de cellule. Fix : scanner le contenu de la CELLULE hote (GetParentTableCell().GetContent()) quand l'injecte est intra-cellule. Regression latente depuis build .4 (branche merge A6). NB : autres regles A intra-cellule (bord->nouveau ¶, espaces, post-sel) encore non portees (host-detection l.852 = top-level only). — 2026-07-16.7 — A6-postsel : la post-selection du chemin BLOC couvrait TOUT le 1er/dernier ¶ d'injection (prefixe/suffixe hote inclus). Fix sans sentinelle : (1) INSERT dont le 1er para plain fusionne le prefixe (firstParaMergedInline) demarre la selection au point de fusion (preSelStart), comme le chemin inline A2/A4 ; (2) mergedTrailingLen mesure (span de position, pas char) le suffixe fusionne dans cleanupTrailingBlockPara -> la selection exclut le suffixe hote. Replace-start deja OK (preSelStart). — 2026-07-16.6 — A3 : extraction d'une selection partielle finissant en fin de ¶ (souris = marque ¶ \r\n dans GetText) -> strip du \r\n traînant de rangeText avant le clip (sinon indexOf echoue et le ¶ ENTIER est extrait). — 2026-07-16.5 — A8 : garde de perf sur ¶ top-level (hors cellules) + backstop >500 ; corrige la perte de md au select-all sur doc a tableaux. — 2026-07-16.4 — §5bis A6 : dernier para injecté fusionne le suffixe (merge dans cleanupTrailingBlockPara, formatage preserve). — 2026-07-16.3 — §5bis A6 : insertion multi-¶ au milieu -> inline splice (1er para fusionne prefixe, DERNIER fusionne suffixe). — 2026-07-16.2 — §5bis règle d'insertion (UAT A1/A5) : collage en fin de ¶ non-vide -> NOUVEAU ¶ (spacer trick) au lieu de fusion inline. — 2026-07-16.1 — fix(§4quater): mixed cross-table<->paragraph REPLACE no longer corrupts a top-of-body table under header/footer position collision (H2/replace). Root cause: the non-table-paragraph classification used a raw-position test against GetAllTables (incl. header/footer tables) -> top-of-body paragraph misclassified as in-table -> mixed in-place path skipped -> destructive full-range InsertContent deleted a table row. Fix: element-based GetParentTableCell() membership test. — 2026-07-15.3 dev-probe: probeTables hook (GetAllTables position-collision diagnostic for header/footer-table docs, flag-gated, inert in prod; harness §4quater fixture calibration) — 2026-07-15.2 fix: insert-after-table via body elements + intra_cell only when whole selection is in the cell — header/footer table position-collision (no_cell_match false-positive -> not_involved fall-through; cross-table cell-coord crash -> table-identity filter + GetCell bounds guard; not_involved no longer breaks table scan) — MERGE of feat/image-reinjection into feat/scribe-in-right-panel: combines the \"Assistant\" ribbon tab (2026-07-06.4 — two explicit Inline/Side-panel buttons + Ctrl+Maj+I hints, native OO AI plugin hidden host-side) with the image re-injection chantier (2026-07-09.6 — save-fidelity fix: recalculate=true so the FromJSON+AddDrawing blip is transmitted to the co-editing/x2t save = <a:blip r:embed> + a word/media part; single undo via History.TurnOff/On; live render via blip=ret.url + insert-free warming). See .planning/phases/28-image-reinjection-plugin-only/ + debug/resolved/inject-blip-lost-at-save.md.";
  ...
  // Only the version TOKEN: SCRIBE_BUILD is a full changelog kept as documentation
  // and cache-bust source, but dumping it to the console floods it (it was logged
  // again on every selection change from init()).
  log("build " + SCRIBE_BUILD.split(" ")[0]);
  ...
  function buildAndInject(md, mode, fallbackHtml) {
    ...
    // Deferred re-selection of a whole freshly-INSERTED table. Runs in its OWN callCommand
    // AFTER the injection callCommand's recalc, when the inserted table's cell positions have
    // settled (inside the injection callCommand doc.GetRange over the corner cells stops one
    // cell short — see the flag site). Anchors on the current selection's start (selectByRefs
    // left it at the table's first cell), finds the enclosing table, and selects it corner-to-
    // corner via integer positions. Guarded so it only fires when the selection genuinely
    // starts inside a table (a mixed text+table insert whose selection starts in the text ¶ is
    // left alone). Inert cost when there's nothing to do.
    function reselectFullTableAfterInsert() {
      window.Asc.plugin.callCommand(function() {
        var doc = Api.GetDocument();
        var cur = doc.GetRangeBySelect ? doc.GetRangeBySelect() : null;
        if (!cur) return;
        // Find the enclosing table ELEMENT-based (a selected ¶ -> its cell -> its table), NOT by
        // matching GetAllTables ranges by position: header/footer letterhead tables collide in
        // position with the body (§4quater), so a position match can land on the wrong table and
        // over-select across the original + clone (H1). GetParentTableCell()/GetParentTable() is
        // the ground truth of what the selection is actually in.
        var paras = cur.GetAllParagraphs ? cur.GetAllParagraphs() : [];
        var tbl = null;
        for (var pi = 0; pi < paras.length; pi++) {
          var _pc = paras[pi].GetParentTableCell ? paras[pi].GetParentTableCell() : null;
          if (_pc && _pc.GetParentTable) { tbl = _pc.GetParentTable(); if (tbl) break; }
        }
        if (!tbl) return; // selection not inside a table (e.g. mixed text+table) — leave as is
        // Merged PARTIAL insert (T2b/T2c): the inserted clone is the whole table but only
        // the originally-selected cells carry injected content. Select JUST that cell
        // rectangle, ELEMENT-based (ExpandTo of the top-left cell's first ¶ range and the
        // bottom-right cell's last ¶ range — composes across cell/merge boundaries, unlike
        // doc.GetRange(int,int) which is L#2-fragile and stops short over merges — that is
        // exactly why the whole-table corner-to-corner branch mis-selected T2b/T2c). The
        // full clone itself is unchanged (§4bis reduction under merges = separate chantier).
        var pcJson = Asc.scope.__partialInsertCells;
        var pc = null; try { pc = pcJson ? JSON.parse(pcJson) : null; } catch (e) { pc = null; }
        if (pc && pc.length) {
          var minR = pc[0].r, maxR = pc[0].r, minC = pc[0].c, maxC = pc[0].c;
          for (var pci = 1; pci < pc.length; pci++) {
            if (pc[pci].r < minR) minR = pc[pci].r; if (pc[pci].r > maxR) maxR = pc[pci].r;
            if (pc[pci].c < minC) minC = pc[pci].c; if (pc[pci].c > maxC) maxC = pc[pci].c;
          }
          var pTl = tbl.GetCell(minR, minC), pBr = tbl.GetCell(maxR, maxC);
          var pTlc = pTl ? pTl.GetContent() : null, pBrc = pBr ? pBr.GetContent() : null;
          if (!pTlc || pTlc.GetElementsCount() === 0 || !pBrc || pBrc.GetElementsCount() === 0) return;
          var pTlRange = pTlc.GetElement(0).GetRange();
          var pBrRange = pBrc.GetElement(pBrc.GetElementsCount() - 1).GetRange();
          var pRg = (pTlRange && pBrRange && pTlRange.ExpandTo) ? pTlRange.ExpandTo(pBrRange) : null;
          if (pRg && pRg.Select) pRg.Select();
          return;
        }
        var lr = tbl.GetRowsCount() - 1;
        var tlco = tbl.GetCell(0, 0).GetContent();
        var brc = tbl.GetCell(lr, tbl.GetRow(lr).GetCellsCount() - 1);
        var brco = brc ? brc.GetContent() : null;
        if (!tlco || tlco.GetElementsCount() === 0 || !brco || brco.GetElementsCount() === 0) return;
        var s = tlco.GetElement(0).GetRange().GetStartPos();
        var e = brco.GetElement(brco.GetElementsCount() - 1).GetRange().GetEndPos();
        if (s >= 0 && e > s) { var rg = doc.GetRange(s, e); if (rg) rg.Select(); }
    ...
    window.Asc.plugin.callCommand(function() {
      ...
      // Cells of a MERGED partial-table INSERT to re-select after the doc settles. Set in
      // the merged-partial insert branch below and RETURNED to the plugin side (Asc.scope
      // writes inside a callCommand do NOT propagate to a sibling callCommand — each gets a
      // fresh copy from the plugin side — which is why __fullTableInsert is set AND read in
      // THIS callCommand then returned, never read from a later one). The plugin callback
      // re-publishes these on Asc.scope before invoking reselectFullTableAfterInsert.
      var partialMergedInsertCells = null;
      ...
      // Count of image markers the fragment referenced but that resolve to no drawing
      // in this document (see the image_placeholder branch): reported so the loss is
      // never silent.
      ...
      // LAST-RESORT: build a plain table from the [CELL:r,c] markers alone.
      // Needed by the SIDE-PANEL (chat) flow: its context is extracted in "document"
      // mode, which emits [TABLE:N]/[CELL:r,c] markers but produces NEITHER
      // tableSnapshots NOR tableDocIndices (only SELECTION extraction does). With no
      // snapshot and no original table, reconstructTable used to return null and all
      // call sites did `continue` → the table vanished silently while the text around
      // it was inserted (the "Insérer/Remplacer perd les tableaux" bug).
      // The grid size is derived from the markers themselves. The original's
      // formatting (fonts, column widths, merges) is NOT recoverable on this path —
      // content fidelity is deliberately preferred over silent loss.
      function createTableFromMarkers(cells) {
        if (!cells || cells.length === 0) return null;
        var maxR = 0, maxC = 0;
        for (var mi = 0; mi < cells.length; mi++) {
          if (cells[mi].r > maxR) maxR = cells[mi].r;
          if (cells[mi].c > maxC) maxC = cells[mi].c;
        }
        try {
          var mt = Api.CreateTable(maxR + 1, maxC + 1); // (LIGNES, COLONNES) — cf. note ci-dessus
          if (!mt) return null;
          // Mirror the markdown-pipe table styling so both paths look alike.
          mt.SetWidth("percent", 100);
          mt.SetTableBorderTop("single", 4, 0, 0, 0, 0);
          mt.SetTableBorderBottom("single", 4, 0, 0, 0, 0);
          mt.SetTableBorderLeft("single", 4, 0, 0, 0, 0);
          mt.SetTableBorderRight("single", 4, 0, 0, 0, 0);
          mt.SetTableBorderInsideH("single", 4, 0, 0, 0, 0);
          mt.SetTableBorderInsideV("single", 4, 0, 0, 0, 0);
          // NB : pas de log() ici — dans le bac a sable d'un callCommand il peut lever
          // une ReferenceError, que le catch ci-dessous transformerait en `return null`
          // (= tableau perdu). Piege deja paye sur ce projet.
          return mt;
        } catch (ctErr) {
          return null;
        }
      ...
      // Fill a reconstructed/rebuilt table's cells from the parsed markers.
      function fillTableCells(tbl, parsedCells, fonts) {
        for (var fi = 0; fi < parsedCells.length; fi++) {
          var fc = parsedCells[fi];
          var fCell = tbl.GetCell(fc.r, fc.c);
          if (!fCell) continue;
          var fCc = fCell.GetContent();
          if (!fCc || fCc.GetElementsCount() === 0) continue;
          var ff = fonts[fc.r + "," + fc.c] || {};
          replaceCellContent(fCc, fc, ff.family, ff.size);
        }
      }

      function reconstructTable(ptIndex, origTable, cells) {
        ...
        return createTableFromMarkers(cells);
      }
        ...
        for (var tci = 0; tci < parsedTables.length; tci++) {
          ...
          if (mode === "replace") {
            ...
            if (isStructuralFull) {
              ...
              var repClone = reconstructTable(ptIndex, origTable, ptCells);
              ...
              // SIDE-PANEL (chat) flow: the fragment carries [TABLE:N] markers but no
              // table of the document is being edited (no snapshot, no doc index), so
              // there is nothing to modify in place. Build the table from the markers
              // and let it be inserted — otherwise Replace dropped it silently while
              // inserting the surrounding text.
              var repNew = reconstructTable(ptIndex, null, ptCells);
              if (!repNew) { tableClones[ptIndex] = null; continue; }
              fillTableCells(repNew, ptCells, cFonts);
              tableClones[ptIndex] = repNew;
            } else {
            ...
            var reducedClone = reconstructTable(ptIndex, origTable, ptCells);
            ...
            if (!tableHasMerge(reducedClone)) {
              ...
              // Merged partial insert (T2b/T2c): the clone is the WHOLE table but only
              // selectedCellCoords carry injected content. Record them (RETURNED to the
              // plugin, then re-published on Asc.scope) so the deferred post-insert
              // re-selection targets JUST those cells in the clone — not the whole clone,
              // which selectByRefs selects by default. Band-aid (a) for the merged partial
              // case; the full clone (b, §4bis reduction under merges) stays as-is. Single
              // partial table assumed (harness corpus); a later partial table overwrites.
              partialMergedInsertCells = selectedCellCoords || [];
            }
          } else {
            // Full Insert — reconstruct via FromJSON + modify all cells
            var clone = reconstructTable(ptIndex, origTable, ptCells);
            if (!clone) continue;
      ...
      for (var i = 0; i < blocks.length; i++) {
        ...
        if (block.type === "heading") {
          ...
          // Api.CreateTable prend (LIGNES, COLONNES) dans cette build OO — l'ordre inverse
          // de ce que le nom des variables suggerait. Le code passait (nCols, nRows) :
          // invisible sur un tableau CARRE (tous les cas testes l'etaient), mais tout
          // tableau NON CARRE levait "Row/Col index out of bounds" et faisait echouer
          // TOUTE l'injection (texte compris). Prouve en live : 3 col x 2 lignes.
          var table = Api.CreateTable(nRows, nCols);
          ...
          if (added) {
            ...
            // The marker names no drawing we can resolve (typically a fragment coming
            // from the side-panel chat, whose context was extracted in "document" mode
            // where names are never stamped). Dropping the paragraph here made the image
            // vanish SILENTLY while the text around it was inserted. Leave a visible,
            // undoable trace instead so the user can see something did not make it.
            log("Scribe: unresolved image " + block.name + " -> visible placeholder");
            unresolvedImages = unresolvedImages + 1;
            var missPara = Api.CreateParagraph();
            addRunsToParagraph(missPara, [{ text: "[image]", italic: true }], srcFontFamily, srcFontSize);
            content.push(missPara);
          }
      ...
      // Post-operation: full-table INSERT — flag for a DEFERRED whole-table re-selection.
      // selectByRefs (run mid-mutation, right after InsertContent) leaves the LAST cell out of
      // the selection: the freshly-inserted table's cell positions are NOT yet settled inside
      // this callCommand, so BOTH ExpandTo of cell-¶ ranges AND doc.GetRange(start,end) stop one
      // cell short — even off a fresh table whose cells already report correct positions here
      // (probe-confirmed: the identical doc.GetRange covers all N cells only once the doc has
      // recalculated, i.e. in a SUBSEQUENT callCommand). The Replace path doesn't hit this
      // because it replaces an EXISTING table at stable positions. So the fix cannot live in
      // this callCommand: signal the callback to re-select in a follow-up callCommand (see
      // reselectFullTableAfterInsert). Gated to content ENDING in a table so a mixed text+table
      // insert (T4) is untouched; the follow-up further guards on the selection starting inside
      // that table.
      if (mode === "insert" && content.length
          && content[content.length - 1] && content[content.length - 1].GetClassType
          && content[content.length - 1].GetClassType() === "table") {
        Asc.scope.__fullTableInsert = "1";
      }
      ...
      // FromJSON + AddDrawing from the pre-registered media map) — nothing is deferred except
      // the full-table-insert re-selection (positions must settle first — see the callback).
      return JSON.stringify({
        fullTableInsert: (Asc.scope.__fullTableInsert === "1"),
        partialInsertCells: partialMergedInsertCells
      });
    }, false, scribeInjectRecalc, function(ret) {
      // Deferred re-selection of a freshly-INSERTED table, run in the callback AFTER the doc
      // has recalculated (inside the injection callCommand the new table's positions aren't
      // settled — ExpandTo/doc.GetRange over its corner cells stop one cell short). Two cases:
      //  • whole-table insert (T3/H1) → select the whole clone corner-to-corner;
      //  • merged PARTIAL insert (T2b/T2c) → select ONLY the edited cells in the clone.
      // partialInsertCells is RETURNED from the callCommand (Asc.scope writes there don't
      // survive to a sibling callCommand); republish it on Asc.scope HERE (plugin side) so
      // reselectFullTableAfterInsert's own callCommand reads it. Clear it otherwise so a prior
      // insert's value can't leak into a whole-table re-selection.
      try {
        var _fti = ret ? JSON.parse(ret) : null;
        var _pic = _fti && _fti.partialInsertCells;
        if (_pic && _pic.length) {
          Asc.scope.__partialInsertCells = JSON.stringify(_pic);
          reselectFullTableAfterInsert();
        } else if (_fti && _fti.fullTableInsert) {
          Asc.scope.__partialInsertCells = null;
          reselectFullTableAfterInsert();
        }
      } catch (eFti) {}
      callbackFired = true;
  ...
  window.addEventListener("message", function(event) {
    ...
    if (msg.action === "AI_TEXT_ASSISTANT") {
      ...
      // Echo back whoever asked. The plugin has no opinion on it; the host reads it
      // to tell a BUTTON press from a keyboard press, which it must handle
      // differently while the side panel is open.
      castScribeTriggerFresh(msg.source ? { source: msg.source } : undefined);
    }
  });
    ...
    try {
      ...
      window.Asc.plugin.callCommand(buildScribeExtractionResult, false, false, function(resultJson) {
        ...
        try {
          ...
          // Snapshots ToJSON des tableaux du contexte, indexes par [TABLE:N]. L'hote
          // les garde avec le message et ne renvoie QUE ceux reellement references par
          // le fragment insere (le canal retour PANEL_ACTION est plafonne a 1 Mo,
          // contrairement a ce message-ci qui contourne la limite).
          snaps = (parsed && parsed.tableSnapshots) ? parsed.tableSnapshots : null;
        } catch (eParse) {
          error = "parse-error";
        }
        postToAncestors({
  ...
  window.Asc.plugin.init = function(data) {
    ...
    // The build log lives INSIDE this first-init guard: OO calls init() on EVERY
    // selection change (initOnSelectionChanged), so logging here unconditionally
    // flooded the console — once is enough.
    if (!toolbarButtonAdded) {
      log("init() — build " + SCRIBE_BUILD.split(" ")[0]);
      addToolbarButton();
      addAssistantTab();
      watchDocumentGeometry();
  ...
  function buildScribeExtractionResult() {
      ...
      function paragraphToMarkdown(para, clipStartChars, clipEndChars) {
            ...
            if (!runText || runText.length === 0) {
              ...
              if (hasScribeDrawings) {
                ...
                for (var ed = 0; ed < emptyRunDrawings.length; ed++) {
                  ...
                  annotatedParts.push({ text: (Asc.scope.scribeExtractMode === "document") ? "[image]" : ("{{IMG:" + edName + "}}"), raw: true });
            ...
            if (hasScribeDrawings) {
              ...
              if (inlineDrawings.length > 0) {
                ...
                for (var id = 0; id < inlineDrawings.length; id++) {
                  ...
                  annotatedParts.push({ text: (Asc.scope.scribeExtractMode === "document") ? "[image]" : ("{{IMG:" + dName + "}}"), raw: true });
        ...
        for (var fd = 0; fd < paraDrawings.length; fd++) {
          ...
          if (!emittedImg[fdName]) {
            annotatedParts.push({ text: (Asc.scope.scribeExtractMode === "document") ? "[image]" : ("{{IMG:" + fdName + "}}"), raw: true });
      ...
      function getDrawingMarker(para) {
        ...
        // CONTEXT (whole-document) extraction never calls SetName (it must not mutate
        // the document), so a "scribe-img-N" name emitted here exists on NO drawing:
        // it is a FICTIONAL handle. If the LLM echoes it back and the user clicks
        // Insérer/Remplacer in the side panel, injectDrawingInto cannot resolve it and
        // the image is lost. Emit a NON-reinjectable, purely descriptive token instead
        // — the model still knows an image is there, but no unhonourable handle is ever
        // handed out. (SELECTION extraction does SetName, so its markers stay real.)
        var isCtx = (Asc.scope.scribeExtractMode === "document");
        if (trimmed.length === 0 && markers.length > 0) {
          // Block images — one per line
          var result = [];
          for (var m = 0; m < markers.length; m++) {
            result.push(isCtx ? "[image]" : ("![IMG:" + markers[m].name + "](placeholder)"));
          }
          ...
          for (var m2 = 0; m2 < markers.length; m2++) {
            inlineParts.push(isCtx ? "[image]" : ("{{IMG:" + markers[m2].name + "}}"));
          }
      ...
      // CONTEXT extraction: snapshots ToJSON des tableaux du document, indexes par
      // le MEME compteur que les marqueurs [TABLE:N] emis ci-dessous. ToJSON est une
      // operation de LECTURE (aucune mutation) : elle ne viole donc pas la contrainte
      // "l'extraction de contexte ne modifie pas le document" qui interdit SetName.
      // Elle preserve les FUSIONS (gridSpan/vMerge), ce qu'une reconstruction depuis
      // les seuls marqueurs ne peut pas faire. L'hote les conserve avec le message et
      // les renvoie au clic Inserer/Remplacer -> reconstructTable retrouve un snapshot
      // et restitue le tableau a l'identique, meme si le document a change depuis.
      var docTableSnapshots = [];
      function buildDocumentExtractionResult(doc) {
        ...
        for (var i = 0; i < blockCount; i++) {
          ...
          if (ct === "table") {
            ...
            try { docTableSnapshots[tableIndex] = el.ToJSON(true, true); }
            catch (eSnap) { docTableSnapshots[tableIndex] = null; }
        ...
        return JSON.stringify({ md: mdLines.join(""), blockCount: mdParts.length, tableSnapshots: docTableSnapshots });
      }
      ...
      for (var p = 0; p < paragraphs.length; p++) {
        ...
        // A paragraph INSIDE a table cell returns its text with a trailing TAB cell-
        // terminator (para.GetText() → "Alpha\t") that the runs do NOT contain, and that
        // range.GetText() also omits. Left in place it inflates paraText by 1, so a WHOLE-
        // cell selection ("Alpha") reads as SHORTER than paraText ("Alpha\t") → the branch
        // below treats it as partial and sets clipEnd = 1 → the LAST character is dropped
        // from the markdown sent to the LLM ("Alpha" → "Alph"). Only non-last-column cells
        // are affected (the last column ends in \r\n, already stripped). Same family as the
        // A3 \r\n fix. Gated to cells so a user-typed trailing tab in a normal ¶ — which the
        // runs DO contain — is not wrongly clipped.
        var inCell = false;
        try { inCell = !!(para.GetParentTableCell && para.GetParentTableCell()); } catch (e) {}
        if (inCell) paraText = paraText.replace(/\t$/, "");
        if ((p === 0 || p === paragraphs.length - 1) && paraText.length > 0) {
          ...
          if (inCell) rangeText = rangeText.replace(/\t$/, "");
      ...
      // Selection screen rect (editor-window px) for the "under-selection" floating
      // button — rides THIS extraction callCommand (no extra callCommand → redo stack
      // stays intact). Api.GetSelectionScreenRect is our sdkjs patch (word/apiBuilder.js);
      // guarded so an unpatched SDK just yields null (button silently stays hidden).
      var __screenRect = null;
      try {
        if (typeof Api.GetSelectionScreenRect === "function") {
          __screenRect = Api.GetSelectionScreenRect();
        }
      } catch (e) { __screenRect = null; }

      return JSON.stringify({
        text: plainParts.join("\n"),
        md: mdLines.join(""),
        screenRect: __screenRect,
  ...
  function onSelectionExtractResult(resultJson) {
      ...
      if (result && result.empty) {
        ...
        lastSelectedHtml = "";
        lastPolledNonEmpty = false;
        // Hide the under-selection floating button (independent of the panel).
        // Fallback only: once the patched SDK pushes geometry, it owns this.
        if (!selectionGeometryPushed) castSelectionGeometry(null, false);
      ...
      // Under-selection floating button — FALLBACK path for an unpatched SDK. Emits
      // regardless of the panel being open, so the button tracks the selection
      // independently. Skipped once the SDK pushes geometry itself
      // (event_onSelectionGeometryChanged), which is both far earlier than this
      // debounced extraction and able to follow scroll/zoom. Rect is null for
      // image-only selections (the bounds are text-only) → button hidden there (MVP).
      if (!selectionGeometryPushed) {
        var __hasSel = !!(lastSelectedText && lastSelectedText.length);
        castSelectionGeometry(result.screenRect || null, __hasSel);
      }
  ...
  // Emit the SELECTION_GEOMETRY intent — a light {rect,hasText} payload the host uses
  // to place the "under-selection" floating Scribe button. Deliberately NOT folded
  // into SELECTION_CHANGED (which carries heavy html/markdown/table data and is gated
  // on the panel being open). rect is in editor-window client px (see the sdkjs patch
  // asc_GetSelectionScreenRect); the host adds the editor iframe offset.
  // A CLICK IS NOT A SELECTION. Clicking to move the caret made the button flash:
  // OO leaves Selection.Use set on a bare click (Selection_SetStart on mouse down,
  // Start === End on mouse up), so the sdkjs patch — which computes geometry under
  // IsSelectionUse() — happily reported the CARET box, and `hasText` was derived
  // from "there is a rect", which was true.
  //
  // IsSelectionEmpty() on the logic document is the honest test, and it is readable
  // from here (same-origin plugin frame, see getParentDocument above) without any
  // callCommand. The zero-width test is the fallback for an SDK whose internals
  // moved: a caret box has no width, a real selection always does.
  function isSelectionReallyEmpty(rect) {
    try {
      var logicDoc = window.parent.Asc.editor.WordControl.m_oLogicDocument;
      if (logicDoc && typeof logicDoc.IsSelectionEmpty === "function")
        return !!logicDoc.IsSelectionEmpty();
    } catch (e) {}
    return !rect || !(rect.width > 0);
  }

  function castSelectionGeometry(rect, hasText) {
    var isReal = !!hasText && !!rect && !isSelectionReallyEmpty(rect);
    castIntent(
      "SELECTION_GEOMETRY",
      { rect: isReal ? rect : null, hasText: isReal },
      true
    );
  }

  // Set once the patched SDK pushes geometry on its own (see below). From then on the
  // slow piggyback path stops emitting, so a stale rect computed during the debounced
  // extraction can never overwrite a fresh pushed one.
  var selectionGeometryPushed = false;

  // PUSHED selection geometry — the fast path.
  //
  // The fallback path rides the debounced heavy extraction, which is ~1s late here: the
  // plugin runs in a BACKGROUND iframe whose setTimeout is heavily throttled, and the
  // extraction itself walks the whole selection. That lag was visible as the button
  // appearing, moving and disappearing well after the selection changed. It also could
  // not track a pure SCROLL or ZOOM at all, since no selection change occurs then.
  //
  // Firing our own lightweight callCommand more often was NOT an option: callCommand
  // truncates the redo stack (same reason the rect was piggybacked in the first place).
  // So the SDK patch pushes the geometry instead — no callCommand, no redo damage, and
  // it fires on scroll/zoom too. Falls back silently on an unpatched SDK.
  window.Asc.plugin.event_onSelectionGeometryChanged = function(data) {
    selectionGeometryPushed = true;
    castSelectionGeometry(data && data.rect, data && data.hasText);
  };

  // ---- DOCUMENT_GEOMETRY: where the document area (and the page) are on screen ----
  //
  // The side-panel floating button has to live INSIDE the document area. Drive cannot
  // find that area on its own: the ribbon, the rulers and the right-hand icon strip
  // are painted inside the OO iframe (another origin), and the pages themselves are
  // drawn on a CANVAS, so there is no per-page DOM node to measure either. A hard
  // coded offset is not an option — it is wrong the moment the user toggles the
  // compact ribbon.
  //
  // What exists is `id_viewer` / `id_viewer_overlay`: two sibling controls anchored to
  // all four sides of the same container (word/Drawing/HtmlPage.js), so their box IS
  // the document area, with every piece of OO chrome outside it. This plugin frame is
  // same-origin with the editor page and OO creates it WITHOUT a `sandbox` attribute
  // (common/plugins.js), so a plain DOM read on window.parent is available. Cost: no
  // sdkjs patch, and — the point — no `callCommand`, which would truncate the user's
  // redo stack.
  //
  // The PAGE box is a refinement on top: the white page is narrower than the viewer,
  // and "just right of the page" is not "right of the viewer". It comes from the
  // drawing document, which is NOT public API — hence the try/catch and the null: the
  // host then falls back to the viewer box, which is already enough to clear the
  // chrome. Everything below degrades to "no push at all" rather than to a wrong box.
  function getParentDocument() {
    try {
      var doc = window.parent && window.parent.document;
      return doc && doc.getElementById ? doc : null;
    } catch (e) {
      return null; // cross-origin (shouldn't happen, plugins are served by OO itself)
    }
  }
  ...
  // id_viewer first: OO sets the overlay to display:none whenever it hides it, and a
  // hidden element measures as an empty box.
  function getViewerRect() {
    var doc = getParentDocument();
    if (!doc) return null;
    return readRect(doc.getElementById("id_viewer"))
        || readRect(doc.getElementById("id_viewer_overlay"));
  }

  function getPageRect() {
    try {
      var dd = window.parent.Asc.editor.WordControl.m_oDrawingDocument;
      var idx = dd.m_lCurrentPage;
      if (!(idx >= 0) || !dd.m_arrPages || !dd.m_arrPages[idx]) return null;
      var page = dd.m_arrPages[idx];
      // ConvertCoordsToCursor3(..., true) yields the EDITOR-WINDOW frame — the same
      // frame as the selection rect, because it folds in m_oWordControl.X/Y and the
      // ruler offset exactly as GetSelectionScreenRect does.
      var tl = dd.ConvertCoordsToCursor3(0, 0, idx, true);
      var br = dd.ConvertCoordsToCursor3(page.width_mm, page.height_mm, idx, true);
      if (!tl || !br || tl.Error || br.Error) return null;
      var w = br.X - tl.X;
      var h = br.Y - tl.Y;
      if (!(w > 0) || !(h > 0)) return null;
      return { left: tl.X, top: tl.Y, width: w, height: h };
    } catch (e) {
      return null; // internals moved (OO upgrade) -> host falls back to the viewer box
    }
  }

  var lastDocumentGeometryKey = "";

  function castDocumentGeometry() {
    var viewer = getViewerRect();
    if (!viewer) return;
    var page = getPageRect();
    var key = [
      viewer.left, viewer.top, viewer.width, viewer.height,
      page ? page.left : "-", page ? page.width : "-"
    ].join(",");
    if (key === lastDocumentGeometryKey) return;
    lastDocumentGeometryKey = key;
    castIntent("DOCUMENT_GEOMETRY", { viewer: viewer, page: page || null }, true);
  }

  var documentGeometryWatched = false;

  // Subscribed once from init(). Every source of change is an event — no polling:
  // a background iframe's setTimeout is throttled hard, which is what made the old
  // piggyback path lag in the first place.
  function watchDocumentGeometry() {
    if (documentGeometryWatched) return;
    var doc = getParentDocument();
    if (!doc) return;
    documentGeometryWatched = true;

    castDocumentGeometry();

    // Window resize, compact-ribbon toggle, OO right panel opening: all of them
    // RESIZE the viewer, so one observer covers the lot. ResizeObserver also fires
    // once on observe, which doubles as a second initial push after layout settles.
    try {
      var viewerEl = doc.getElementById("id_viewer");
      if (viewerEl && window.parent.ResizeObserver) {
        new window.parent.ResizeObserver(function() {
          castDocumentGeometry();
        }).observe(viewerEl);
      }
    } catch (e) {}

    // Zoom does not resize the viewer — it resizes the PAGE inside it, which no DOM
    // event reports. asc_registerCallback APPENDS to the editor's own handler list
    // (word/api.js), it does not replace it.
    try {
      var api = window.parent.Asc.editor;
      api.asc_registerCallback("asc_onZoom", castDocumentGeometry);
      api.asc_registerCallback("asc_onZoomChange", castDocumentGeometry);
    } catch (e) {}
  }
